...
from .services.question_banks import process_question_generation_queue, process_question_generation_task
...
@csrf_exempt
def internalBuildJobInterviewBlueprint(request):
    if request.method != 'POST':
        return JsonResponse({'Success': False, 'Error': 'Only POST is allowed.', 'Data': None}, status=405)
    ...
    try:
        generation_job_id = int(payload.get('generation_job_id') or 0) or None
        job_id = int(payload.get('job_id') or 0) or None
    except (TypeError, ValueError):
        return JsonResponse({'Success': False, 'Error': 'Blueprint task payload is invalid.', 'Data': None}, status=400)

    result = process_job_interview_blueprint_task(generation_job_id, job_id)
    status_code = 200 if result.get('ok') else 500
    return JsonResponse({'Success': result.get('ok', False), 'Error': None if result.get('ok') else result.get('message'), 'Data': result}, status=status_code)


@csrf_exempt
def internalGenerateSkillQuestionBank(request):
    if request.method != 'POST':
        return JsonResponse({'Success': False, 'Error': 'Only POST is allowed.', 'Data': None}, status=405)
    ...
    try:
        generation_job_id = int(payload.get('generation_job_id') or 0) or None
        skill_id = int(payload.get('skill_id') or 0) or None
        task_type = str(payload.get('task_type') or '').strip() or None
    except (TypeError, ValueError):
        return JsonResponse({'Success': False, 'Error': 'Question generation task payload is invalid.', 'Data': None}, status=400)

    result = process_question_generation_task(generation_job_id, skill_id, task_type)
    status_code = 200 if result.get('ok') else 500
    return JsonResponse({'Success': result.get('ok', False), 'Error': None if result.get('ok') else result.get('message'), 'Data': result}, status=status_code)


@csrf_exempt
def internalProcessQuestionGenerationQueue(request):
    try:
        logger.info('Question generation worker endpoint started')
        if request.method != 'POST':
            return JsonResponse({'Success': False, 'success': False, 'Error': 'Only POST is allowed.', 'Data': None}, status=405)

        shared_secret = getattr(settings, 'CLOUD_TASKS_SHARED_SECRET', '').strip()
        provided_secret = request.META.get(INTERVIEW_REMINDER_TASK_HEADER, '')
        if not shared_secret or not provided_secret or not hmac.compare_digest(shared_secret, provided_secret):
            return JsonResponse({'Success': False, 'success': False, 'Error': 'Unauthorized task request.', 'Data': None}, status=403)

        try:
            payload = json.loads(request.body.decode('utf-8') or '{}')
        except Exception:
            return JsonResponse({'Success': False, 'success': False, 'Error': 'Invalid request payload.', 'Data': None}, status=400)

        try:
            requested_limit = int(payload.get('limit') or 0)
        except (TypeError, ValueError):
            return JsonResponse({'Success': False, 'success': False, 'Error': 'Question generation queue payload is invalid.', 'Data': None}, status=400)

        configured_limit = max(1, int(getattr(settings, 'INTERVIEW_QUESTION_BANK_WORKER_LIMIT', 1)))
        limit = min(configured_limit, requested_limit) if requested_limit > 0 else configured_limit
        results = process_question_generation_queue(limit=limit)
        ...
        logger.info(
            'Question generation worker completed limit=%s processed=%s failed=%s',
            limit,
            len(results),
            failed_count,
        )
        return JsonResponse({'Success': True, 'success': True, 'Error': None, 'Data': summary}, status=200)
    except Exception as exc:
        logger.exception('Question generation worker endpoint failed')
        return JsonResponse({
            'Success': False,
            'success': False,
            'Error': str(exc)[:500],
